...
if($_POST['idh']){	
  ...
  if($_POST['idh']=='-'){
	...
	if($cSubstr == 5){
	  list($sks, $idh, $nod, $idm, $idk, $idjad) = explode('-', $_POST['idh']);
	  ...
	  list($sks, $idh, $nod, $nods, $idm, $idk, $idjad) = explode('-', $_POST['idh']);
	  $rf = mysqli_query($dbc, "select id_fak from matkulmaster where id_matkul=$idm");
	  list($idf) = mysqli_fetch_row($rf);
	  $qn = "select id_fak from dosen where nod=$nod";
	  $rn = mysqli_query($dbc, $qn);
    ...
    $q = "select jam_kul from tabeljam where id_fak=$idf and id_TA={$_SESSION['id_ta']} order by jam_kul asc";
	...
	if(((isset($idfn) and !isset($idfns)) and $idfn==$idf) or ((isset($idfn) and isset($idfns)) and ($idfn==$idf and $idfns==$idf))){
	  ...
	  if(isset($nod) and !isset($nods)){
		...
		if($jsx!=0 and $idhx==$idh){
	      ...
	      echo'<option value="'.$nod.'-'.$jsx.'-'.$jex.'-'.$idh.'-'.$mjx.'-'.$mmx.'-'.$sjx.'-'.$smx.'-'.$idk.'-'.$idm.'-'.$idjad.'" style="background:#606060; color:white;">'.$jsx.' - '.$jex.' &nbsp;/&nbsp; '.$mjl.':'.$mml.' - '.$sjl.':'.$sml.'</option>';
		}
	    ...
	    for($i=1;$i<=$jPart;$i++){
	      ...
	      $ql = "select id_lkl from lokal where id_TA={$_SESSION['id_ta']} and id_fak={$_SESSION['iddf']}";
	      $rl = mysqli_query($dbc, $ql);
	      while(list($idl) = mysqli_fetch_row($rl)){
	       $cIDL += 1;
		    while(list($k, $qRange) = each($range)){
		      $q = "select * from jadwal where id_TA={$_SESSION['id_ta']} and id_fak={$_SESSION['iddf']} and id_hari=$idh and id_lkl=$idl and ($qRange between jk_start and jk_end)";
		      $r = mysqli_query($dbc, $q);
	      ...
	      if(($struck!=$cIDL) and ($pdStruck==0) and ($kStruck==0)){
	        $qjs = "select mulai_jam, mulai_menit from tabeljam where id_TA={$_SESSION['id_ta']} and id_fak={$_SESSION['iddf']} and jam_kul=$sJam";
		    $rjs = mysqli_query($dbc, $qjs);
		    $qje = "select selesai_jam, selesai_menit from tabeljam where id_TA={$_SESSION['id_ta']} and id_fak={$_SESSION['iddf']} and jam_kul=$eJam";
	        ...
	        echo'<option value="'.$nod.'-'.$sJam.'-'.$eJam.'-'.$idh.'-'.$mj.'-'.$mm.'-'.$sj.'-'.$sm.'-'.$idk.'-'.$idm.'-'.$idjad.'">'.$sJam.' - '.$eJam.' &nbsp;/&nbsp; '.$mjl.':'.$mml.' - '.$sjl.':'.$sml.'</option>';
	      }
		...
		if($jsx!=0 and $idhx==$idh){
		  ...
		  if(strlen($sjx)==1){ $sjl = 0 . $sjx; }else{ $sjl = $sjx; }
		  if(strlen($smx)==1){ $sml = $smx . 0; }else{ $sml = $smx; }
	      echo'<option value="'.$nod.'-'.$nods.'-'.$jsx.'-'.$jex.'-'.$idh.'-'.$mjx.'-'.$mmx.'-'.$sjx.'-'.$smx.'-'.$idk.'-'.$idm.'-'.$idjad.'" style="background:#606060; color:white;">'.$jsx.' - '.$jex.' &nbsp;/&nbsp; '.$mjl.':'.$mml.' - '.$sjl.':'.$sml.'</option>';
		}		
	    ...
	    for($i=1;$i<=$jPart;$i++){
	      ...
	      if(($struck!=$cIDL) and ($pdStruck==0) and ($pdStrucks==0) and ($kStruck==0)){
		    ...
		    if(strlen($sj)==1){ $sjl = 0 . $sj; }else{ $sjl = $sj; }
		    if(strlen($sm)==1){ $sml = $sm . 0; }else{ $sml = $sm; }
	        echo'<option value="'.$nod.'-'.$nods.'-'.$sJam.'-'.$eJam.'-'.$idh.'-'.$mj.'-'.$mm.'-'.$sj.'-'.$sm.'-'.$idk.'-'.$idm.'-'.$idjad.'">'.$sJam.' - '.$eJam.' &nbsp;/&nbsp; '.$mjl.':'.$mml.' - '.$sjl.':'.$sml.'</option>';
	      }
	  ...
	  if(isset($nod) and !isset($nods)){
	    ...
	    for($i=1;$i<=$jPart;$i++){
          ...
          $eJam = ($i+$sks)-1;
          $sJam = $i;
		  $qidf = "select id_fak from matkulmaster where id_matkul=$idm";
		  $ridf = mysqli_query($dbc, $qidf);
		  list($idf) = mysqli_fetch_row($ridf);
	      $qjs = "select mulai_jam, mulai_menit from tabeljam where id_TA={$_SESSION['id_ta']} and id_fak=$idf and jam_kul=$sJam";
	      $rjs = mysqli_query($dbc, $qjs);
	      $qje = "select selesai_jam, selesai_menit from tabeljam where id_TA={$_SESSION['id_ta']} and id_fak=$idf and jam_kul=$eJam";
	      $rje = mysqli_query($dbc, $qje);
	      list($mj, $mm) = mysqli_fetch_row($rjs);
	      list($sj, $sm) = mysqli_fetch_row($rje);
	      $start = (100 * $mj) + $mm + 1;
	      ...
	      $ql = "select id_lkl from lokal where id_TA={$_SESSION['id_ta']} and id_fak=$idf";
	      $rl = mysqli_query($dbc, $ql);
	      while(list($idl) = mysqli_fetch_row($rl)){
	        $cIDL += 1;
		    while(list($k, $qRange) = each($range)){
		      $q = "select * from jadwal where id_TA={$_SESSION['id_ta']} and id_fak=$idf and id_hari=$idh and id_lkl=$idl and ($qRange between $dbStart and $dbEnd)";
		      $r = mysqli_query($dbc, $q);
		      if(mysqli_num_rows($r)>0){
		        $struck += 1;
			    break;
